...
class settings:
    ...
    ffmpeg_render_args = {
        'hwaccel': None,  # Hardware acceleration API [str = None]
        'hwaccel_device': None,  # Hardware acceleration device (GPU) ID [int = None]
        'c:v': 'libsvtav1',  # Video codec [str]
        'minrate:v': '2m',  # Minimum video bit rate [str]
        'maxrate:v': '10m',  # Maximum video bit rate [str]
        'quality': 'high',  # Quality preset [str]
        'b:v': 0,  # Video bit rate (0 = VBR) [int]
        'tune': 'stillimage',  # Tune preset [str]
        'framerate': '60',  # Frame rate [str]
        'threads': 0,  # Number of threads [int = 0]
        'tile_columns': 2,  # Number of tile columns [int]
        'tile_rows': 2,  # Number of tile rows [int]
        'profile:v': 'main',  # Video profile [str]
        'pred': None,  # Prediction mode [int]
        'b_strategy': None,  # B-frames strategy [int]
        'c:a': 'libopus',  # Audio codec [str]
        'b:a': '128kbps',  # Audio bit rate [str]
...
def render_file(input: pathlib.Path, output: pathlib.Path, now_ep_number: int, total_eps_quantity: int) -> int:
    ...
    ffmpeg_other_args.append(f'"{output.absolute()}"')

    print(f'INPUT: {input.absolute()}')
